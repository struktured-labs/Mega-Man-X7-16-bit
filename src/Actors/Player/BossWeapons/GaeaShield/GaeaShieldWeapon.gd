...
# GaeaShield Weapon - Shield orbits player, charged launches rock projectile
...
func fire_regular() -> void:
	...
	set_position_as_character_position(current_shield)

func fire_charged() -> void:
	play(weapon.charged_sound)
	# If shield is active, launch it forward
	if is_instance_valid(current_shield):
		current_shield.expire()
