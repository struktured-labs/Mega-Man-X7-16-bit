...
# WindCutter Charged - Homing wind blade from Wind Crowrang
# Individual homing projectile (8 fired at once by weapon script)

const speed := 320.0
const turn_amount := 0.01
export var tracking_time := 1.5
export var tracker_update_interval := 0.08
...
func _Setup() -> void:
	._Setup()
	set_horizontal_speed(speed * cos(launch_angle))
	set_vertical_speed(speed * sin(launch_angle))
...
func slowly_turn_towards_target(target_speed : Vector2, delta : float) -> void:
	...
	var new_angle = lerp_angle(current_angle, target_angle, delta * 8)
