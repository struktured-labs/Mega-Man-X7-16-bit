...
# Hadangeki (from Gungaroo) - Standing saber wave projectile
# Press alt_fire while standing on ground -> fires energy wave forward
...
func _StartCondition() -> bool:
	if character.is_on_floor() and not character.is_executing("Walk"):
		if not character.is_executing("Dash"):
			if has_ammo():
				return true
	return false
...
func fire_projectile() -> void:
	if projectile_scene:
		...
		proj.global_position = character.global_position + Vector2(
			12 * character.get_facing_direction(), -6)
...
func play_animation_on_initialize():
	character.play_animation("slash1")
